	...
	window.transfer = new Transfer(null, "main", true).listen(function(msg){
		...
		case _const.EVENTS.TEXTMSG:
			...
			if(
				...
				channelAdapter.sendText(messageBuilder.textMessage(data.data, extendMessage.ext));
			}
			break;
...
function initChatEntry(targetUserInfo){
	...
	.then(function(){
		...
		if(targetUserInfo){
			...
			// 发送ext消息，指定接待坐席
			profile.commandMessageToBeSendList.push({ ext: { agentUsername: targetUserInfo.agentUserName } });
